...
namespace structuraltests
{
	...
	public:
		...
		TEST_METHOD(ClientShouldBeEmptyWhenAdapteeHasNoContent)
		{
			...
			//WHEN
			auto client = new structural::TextShapeObjectAdapter(text_view);
		...
		{
			//GIVEN
			auto text_view = new structural::TextView("test");
		...
		{
			//WHEN
			auto client = new structural::TextShapeClassAdapter();
		...
		TEST_METHOD(ClientShouldCreateClassAdapterWithContentLikeAdaptee)
		{
			...
			//WHEN
			auto client = new structural::TextShapeClassAdapter(expected_content);
