...
namespace patternstests
{
	...
	public:
		...
		TEST_METHOD(ShouldCreateEnchantedRoom)
		{
			...
			patterns::Spell expected_spell(expected_message);

			//WHEN
			auto enchanted_room = patterns::EnchantedRoom(expected_room_number, expected_spell);
		...
		TEST_METHOD(ShouldCreateEnchantedRoomFromAnotherEnchantedRoom)
		{
			...
			auto expected_room_number = 11;
			patterns::Spell expected_spell(expected_message);
			auto existing_enchanted_room = patterns::EnchantedRoom(expected_room_number, expected_spell);

			//WHEN
			auto enchanted_room = patterns::EnchantedRoom(existing_enchanted_room);
		...
		TEST_METHOD(ShouldInitializeEnchantedRoom)
		{
			...
			auto enchanted_room = patterns::EnchantedRoom(expected_room_number, spell);

			//WHEN
			patterns::Spell new_spell("abracadabra");
